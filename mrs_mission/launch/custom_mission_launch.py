...
def generate_launch_description():
    ...
    return LaunchDescription([
        Node(
            package='mrs_mission',
            executable='auctioneer_custom_node',
            name='auctioneer_custom_node', # default name. only used if the node did not set its own name in the code (class initialisation)
            parameters=[
                os.path.join(config_dir, 'global_params.yaml'),
                os.path.join(config_dir, 'auctioneer_custom_params.yaml')
            ],
            output='screen'
        ),
        Node(
